...
class MonteCarloResult:
    ...
    def summary(self) -> str:
        fracture_energies = [e for e, o in zip(self.energies, self.outcomes) if o == "FRACTURE"]
        return (
            ...
            f"  Mean energy (frac.) : {np.mean(fracture_energies):.4f}" if fracture_energies else "" + "\n"
    ...
    def run(self, verbose: bool = True) -> MonteCarloResult:
        ...
        for i in range(self.n_runs):
            ...
            if verbose and (i + 1) % max(1, self.n_runs // 10) == 0:
                ...
                print(f"  [{pct:5.1f}%] Run {i+1}/{self.n_runs} — "
...
def plot_monte_carlo(
    ...
    fig.suptitle(f"Monte Carlo Failure Analysis ({mc_result.n_runs} Simulations)",
    ...
    ax = axes[0, 0]
    if np.any(frac_mask):
        ax.hist(energies[frac_mask], bins=25, alpha=0.7, color="#FF4444",
                label="Fracture", edgecolor="white")
    if np.any(arrest_mask):
        ax.hist(energies[arrest_mask], bins=25, alpha=0.7, color="#4488FF",
                label="Arrest", edgecolor="white")
    ax.axvline(np.mean(energies), color="black", linestyle="--", linewidth=1.5,
    ...
    ax = axes[0, 1]
    if np.any(frac_mask):
        ax.hist(path_lens[frac_mask], bins=25, alpha=0.7, color="#FF4444",
                label="Fracture", edgecolor="white")
    if np.any(arrest_mask):
        ax.hist(path_lens[arrest_mask], bins=25, alpha=0.7, color="#4488FF",
                label="Arrest", edgecolor="white")
    ...
    ax.axvline(median_e, color="#FF8800", linestyle="--",
               label=f"Median: {median_e:.3f}")
    ax.set_xlabel("Total Crack Energy")
    ...
    ax.axvline(np.mean(nodes), color="black", linestyle="--",
    ...
    ax = axes[1, 2]
    if np.any(frac_mask):
        ax.scatter(path_lens[frac_mask], energies[frac_mask],
                   c="#FF4444", alpha=0.5, s=20, label="Fracture", edgecolors="none")
    if np.any(arrest_mask):
        ax.scatter(path_lens[arrest_mask], energies[arrest_mask],
                   c="#4488FF", alpha=0.5, s=20, label="Arrest", edgecolors="none")
    ax.set_xlabel("Path Length (pixels)")
...
def export_csv(mc_result: MonteCarloResult, path: str):
    ...
    with open(path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["run", "outcome", "energy", "path_length",
                         "nodes_explored", "time_seconds"])
